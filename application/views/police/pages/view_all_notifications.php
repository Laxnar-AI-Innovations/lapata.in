            ...
            <div class="row justify-content-center">
                <div class="col-lg-6 col-sm-12 col-md-8 card">
                    <div class="card-body text-center py-3">
                        <button type="button" class="btn btn-style-five disabled-on-loading" onclick="markAllAsRead(this)" style="padding:0.5rem!important;border-radius:0!important;" id="markAllAsRead"> Mark all as read</button>
                    </div>
                </div>
            </div>
            <?php } ?>
                        ...
                        <div class="notification-list <?= ($v['status'] == 'Active' ? 'notification-list--unread' : ''); ?>" style="cursor:pointer;" onclick="openNotification('old',<?= $v['Id']; ?>)">
                            <div class="notification-list_content align-items-center">
                                <div class="notification-list_img">
                                    <?php 
                                        if(file_exists($getUserDetail[0]['aadharphoto']))
                                        {
                                    ?>
                                    <img src="<?= base_url(); ?><?= ($getUserDetail[0]['aadharphoto']) ?? 'assets/images/male.png'; ?>" alt="user">
                                    <?php }else{ ?>
                            ...
                            <div class="notification-list_feature-img align-items-center d-flex">
                                <?php 
                                    if(file_exists($getFoundPersonDetail[0]['facematchphoto']))
                                    {
                                ?>
                                <img src="<?= base_url(); ?><?= ($getFoundPersonDetail[0]['facematchphoto']) ?? 'assets/images/male.png'; ?>" alt="user">
                                <?php }else{ ?>
                                <img src="<?= base_url(); ?><?= 'assets/images/male.png'; ?>" alt="user">
                                <?php } ?>
                            </div>
...
<script>
    function markAllAsRead(ele)
    {
        ...
        $.ajax({
            url:'<?php echo base_url(); ?>police/markNotificationAsRead',
