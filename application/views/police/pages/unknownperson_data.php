...
<main id="main" class="main">
    ...
    ?>
    <div class="pagetitle">
      <h1 class="text-center"><span><img src="<?php echo base_url(); ?>assets/icons/unknown.jpg" style="width:50px;height:50px;"></span><b class="ms-3 text-danger">Enter </b>- <?php echo 'Unknown'; ?> Person Details</h1>
    </div>
    <?php }else{ ?>
    <div class="pagetitle">
    ...
    <div class="<?php if($pn != 2){ echo "d-none"; } ?>">
        <div class="row align-items-center">
            <div class="col-lg-7 col-sm-12 col-md-5">
                <h2 class="text-center my-0 text-danger ms-5"><b><span id="personcount"><?php echo (count($data) != 0 ? ((count($data) > 1) ? (count($data).' Persons') : (count($data).' Person')) : 'NO Person'); ?></span> Found...</b></h2>
            </div>
            <div class="col-lg-4 col-sm-12 col-md-5 mt-2">
                <button type="button" onclick="openFaceModal()" class="btn w-100 btn-primary btn-style-five disabled-on-loading" style="padding:0.7rem!important;"> 
                    <img src="<?php echo base_url(); ?>assets/images/faceicon.png" style="width:30px;height:30px;"> Search Person By Face Photo
                </button>
            </div>
        </div>
        	...
        	<div class="side-bar pt-1" >
             ...
             <!--   </div>-->
        	    ...
        	    <div class="row">
                    ...
                    <div class="form-group col-12 col-md-12 col-lg-12 mt-3 form-floating">
                        <button type="button" class="btn btn-style-green disabled-on-loading w-100" onclick="apply(this)"> Search Result</button>
                    </div>
                    <div class="reset-btn">
                ...
                <div class="image-container row" id="paginated-list">
                    <?php
                        foreach($data as $row)
                        {
                          ...
                          ?>
                        <div class="col-lg-4 col-md-6 px-0 px-md-2 col-12 col-search pb-2 pagination-items" data-imageId="<?php echo $row['imageId']??"-" ?>" data-cat="<?php echo $row['DL_no']??"-" ?>" data-goldtype="<?= implode(',',$goldType); ?>" data-goldhuid="<?= implode(',',$goldHUID); ?>" data-gender="<?php echo $row['Gender']??"" ?>" data-vehicle="<?php echo $row['vehicle_no']??"" ?>" data-aadhar="<?php echo $row['aadharNumber']??"" ?>" data-pan="<?php echo $row['PAN']??"" ?>" data-voter="<?php echo $row['voter_id']??"" ?>" data-dl="<?php echo $row['DL_no']??"" ?>" data-age="<?php echo $row['Age']??"" ?>" data-state="<?php echo $row['state']??"" ?>" data-district="<?php echo $row['district']??"" ?>" data-city="<?php echo $row['city']??"" ?>" data-personname="<?php echo $row['person_name']??"" ?>"  data-search="<?php echo $row['DL_no']; ?><?php echo $row['aadharNumber']; ?><?php echo $row['PAN']; ?><?php echo $row['person_name']; ?><?php echo $row['Message']; ?>" >
...
<script>
	...
	function showStartPopup()
	{
	    Swal.fire({
    	    ...
    	    html:'<div class="row mx-0 justify-content-end"><div class="col-auto mt-2"><button type="button" onclick="back(this)" class="btn btn-dark btn-style-grey disabled-on-loading" style="padding:0.7rem!important;background:black!important;"> <i class="fa fa-arrow-circle-left"></i> Back to Dashboard</button></div></div><hr style="border: 1px solid black!important;opacity:0.5;"><div class="row mx-0 mt-3"><div class="col-12"><h6 class="border border-3 border-danger py-2 text-dark"><i class="fa fa-user"></i> <b class="text-danger">Result - </b><b>'+($('#data').val() != 0 ? ($('#data').val() > 1) ? ($('#data').val()+' Persons') : ($('#data').val()+' Person') : 'NO Person')+' </span> Found</b></h6></div></div><div class="row mx-0"><div class="col-12"><h6 class="word mb-0"><b class="text-danger">Note:-</b> <b class="text-dark">It is mandatory to have at least 100 records to see the result. Otherwise you can search by face photo also.</b></h6></div></div><div class="row mx-0"><div class="col mt-2"><button type="button" onclick="checkFacePayment(this)" class="btn btn-primary btn-style-five disabled-on-loading" style="padding:0.7rem!important;"> <img src="<?php echo base_url(); ?>assets/images/faceicon.png" style="width:30px;height:30px;"> Search By Face Photo</button></div><div class="col-sm-12 col-md mt-2"><button type="button" onclick="continueFilter(this)"  class="btn btn-primary w-100 btn-style-four" style="padding:0.7rem!important;"><i class="fa fa-filter"></i> Continue Filter</button></div></div>',
    ...
    document.querySelector('.reset-btn .btn').onclick = () => {
    	window.location.href="<?php echo base_url(); ?>police/unknown_person_filter";
    };
