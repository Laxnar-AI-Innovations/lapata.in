...
<main id="main" class="main">
    ...
    <section class="section profile">
        ...
        <form class="row g-3 needs-validation justify-content-center <?php if($pn != 7){ echo "d-none"; } ?>" method="post" action="<?php echo base_url(); ?>user/searchPersonFace" novalidate id="step7Form">
            <?php 
                $csrf = array(
                  'name' => $this->security->get_csrf_token_name(),
                  'hash' => $this->security->get_csrf_hash()
                    ...
                    <div class="col-2">
                        ...
                        <div class="text-end">
                            <?php 
                                ...
                                {
                            ?>
                            <a href="<?= base_url(); ?>user-dashboard" style="padding: 1rem!important;" class="btn mt-0 btn-style-grey btn-dark disabled-on-loading"><i class="fa fa-arrow-circle-left"></i></a>
                            <?php }else{ ?>
                            <a href="<?= base_url(); ?>user/lapta_person_after_login" style="padding: 1rem!important;" class="btn mt-0 btn-style-grey btn-dark disabled-on-loading"><i class="fa fa-arrow-circle-left"></i></a>
                            <?php } ?>
                        </div>
                        <?php } ?>
                    </div>
...
<script type="text/javascript">
    ...
    function showAccordingly()
    {
        ...
        if(choice == undefined)
        {
            ...
            {
                location.href="<?php echo base_url(); ?>user/search_vehicle_details";
                ...
                if(choice != 3)
                {
                    if(choice != 4)
                    {
                        if(choice != 5)
                        {
                            if(choice == 2)
                            {
                                location.href="<?php echo base_url(); ?>user/search_person_by_identity";         
                            }else{
                            ...
                            {
                                $.ajax({
                                    url:'<?php echo base_url(); ?>user/addFaceSearch',
                                    type:'get',
                                    success:function(res)
                                    {
                                        // location.href="<?php echo base_url(); ?>user/search_person_details?pn=7";                       
                                        location.href="<?php echo base_url(); ?>user/newSearchPerson";                       
                                    }
                                })
...
<script>
    ...
    {
        resetButton.onclick = () => {
        	window.location.href="<?php echo base_url(); ?>user/search_person_details?pn=5";
        };
    }
            ...
            if($('input[name="gender"]').is(':checked'))
            {
                ...
                $.ajax({
                    <?php 
                        ...
                        {
                    ?>
                    url:'<?= base_url(); ?>user/searchPersonFaceNew',
                    <?php }else{ ?>
                    url:$(this).attr('action'),
                    <?php } ?>
                    data:data,
                            ...
                            $.ajax({
                                ...
                                success:function(res)
                                {
                                    <?php 
                                        ...
                                        {
                                    ?>
                                    location.href="<?php echo base_url(); ?>user/showPaymentGateway";      
                                    <?php }else{ ?>
                                        location.href="<?php echo base_url(); ?>user/completeFaceMatchResults?type=<?= $_GET['type']; ?>";
                                    <?php } ?>
                                }
                            })
                        }else{
