...
<main id="main" class="main">
    ...
    <section class="section profile">
      <div class="row justify-content-center">
        <div class="col-xl-5 col-lg-5 col-sm-12 col-md-8 px-0 <?php if($pn != 1){ echo "d-none"; } ?>">
            <div class="card">
                    ...
                    if(isset($_SESSION['vehicleLoggedInId']))
                    {
                        ...
                        if(count($getDetails) > 0)
                        {
                            if($getDetails[0]['isMissing'] == 1)
                            {
                                ...
                                {
                                    $missingvehicle = $this->db->query('select * from missingvehicle where rcNumber="'.$getDetails[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound =0 and expiryDate >= "'.date('d-m-Y').'"')->result_array();   
                                }
                                if($getDetails[0]['searchType'] == 'engineNumber')
                                {
                                    $missingvehicle = $this->db->query('select * from missingvehicle where engineNumber="'.$getDetails[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound =0 and expiryDate >= "'.date('d-m-Y').'"')->result_array();   
                                }
                                if($getDetails[0]['searchType'] == 'chassisNumber')
                                {
                                    $missingvehicle = $this->db->query('select * from missingvehicle where chasisNumber="'.$getDetails[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound =0 and expiryDate >= "'.date('d-m-Y').'"')->result_array();   
                                }
                            }else{
...
text-decoration: underline;"> Click Here</span>
                ...
                <div class="row justify-content-center mt-5">
                    ...
                    <div class="col-md-auto col-lg-auto col-sm-12 mt-2">
                        <a href="<?php echo base_url(); ?>user" class="btn btn-primary save" style="width:-webkit-fill-available;"><i class="fa fa-home"></i> Homepage</a>
                    </div>
                </div>
            </div>
                <?php }else{ 
            ...
            <div class="modal-body">
                ...
                <div class='row justify-content-center text-center mx-0' id="bothcameradiv">
                    <div class='col-lg-6 col-sm-12 col-md-12 mt-3 px-0' id="frontcameradiv">
                        <button type='button' class='w-lg-max-content btn btn-style-green btn-primary' onclick='openFrontCamera()'>Open Front Camera <i class='fa fa-camera fa-spin'></i></button>
                    </div>
                    <div class='col-lg-6 col-sm-12 col-md-6 mt-3 px-0 px-md-2' id="backcameradiv">
                        <button type='button' class='w-lg-max-content btn btn-style-green btn-primary' onclick='openBackCamera()'>Open Back Camera <i class='fa fa-camera fa-spin'></i></button>
                        </div>
                    </div>
                </div>
  ...
  <script type="text/javascript">
    ...
    $('#btnInactiveSave').click(function()
    {
        if($('#mainPhotoSelected').val() != '')
        {
            ...
            if(count == 0)
            {
                ...
                $.ajax({
                    url:'<?php echo base_url(); ?>user/checkInactiveVehicleIdAfterLogin',
                    data:data,
                    processData:false,
                    cache:false,
                    contentType:false,
                    ...
                    success:function(res)
                    {
                        ...
                        {
                            location.href="<?php echo base_url(); ?>user/search_vehicle_details?pn=2";
                        }
                        else{
    ...
    $('#step2Check').click(function()
    {
        ...
        if(count == 0)
        {
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>user/checkVehicleIdAfterLogin',
                data:data,
                processData:false,
                cache:false,
                contentType:false,
    ...
    {
        window.open('<?php echo base_url(); ?>user/search_vehicle_details?pn=2','_blank');
    }
