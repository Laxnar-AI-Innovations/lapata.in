...
<script>
	...
	{
	    $.ajax({
	        url:'<?php echo base_url(); ?>user/checkFacePayment?type=unknown',
	        type:'get',
	        success:function(res)
	        {
	            ...
	            {
	                location.href="<?php echo base_url(); ?>user/manageGallery?type=unknown";
	            }else{
	                $('#payModal').modal('show');
	            }
	        }
    ...
    document.querySelector('.reset-btn .btn').onclick = () => {
    	window.location.href="<?php echo base_url(); ?>user/unknown_person_filter_after_login";
    };
