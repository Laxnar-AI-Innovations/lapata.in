...
<main id="main" class="main">
    ...
    <?php 
        $isUpload = 0;
        if($_SERVER['HTTP_REFERER'] != base_url().'user/lapata_login?pn=3')
        {
    ?>
        <?php 
            if($_SERVER['HTTP_REFERER'] != base_url().'user/manage_lapata')
            {
                $getId = parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY);
                $explode = explode('&',$getId);
        ?>
        <input type="hidden" id="redirect" value="<?php echo base_url().'user/editLapataDetails?'.$explode[0].'&pn=4'; ?>">
        <?php      
            }else{
        ?>
    ...
    <?php }else{ 
        $isUpload = 1;
    ?>
    <input type="hidden" id="redirect" value="<?php echo base_url().'user/lapata_login?pn=4'; ?>">
    <?php } ?>
    <?php 
            ...
            <?php 
                ...
                foreach($getPackages as $k=>$v)
                {
                    if(count($getPersonDetail) > 0)
                    {
                        ...
                        {
                            $getIfAlready = $this->db->query('select * from uploadtransactions where isKyc=1 and userId='.$_SESSION['user_id'].' and idNumber="'.$getPersonDetail[0]['aadharNumber'].'" and packageId=1')->result_array();      
                        }else{
                            $getIfAlready = array();
                        }
                    }else{
                    ...
                    $getAllotedPackages = $this->db->query('select * from allotedPackages where userType="Kyc" and userId='.$_SESSION['user_id'].' and packageId='.$v['Id'].' and complaintType="Missing"')->result_array();
                ...
                <?php } ?>
                    <div class="row" style="justify-content:space-between;align-items:<?= ($v['amount'] == 0) ? 'baseline' : 'end'; ?>;">
                        ...
                        <div class="col-auto px-0">
                            ...
                            ?>
                            <?php 
                                $getIfFreeCredit = $this->db->query('select * from user where uid='.$_SESSION['user_id'])->result_array();
                                $freeCredit = 0;
            ...
            <div class="col-sm-12 col-md-12 col-lg-12 text-center card">
                <div class="card-body p-0 py-3">
                    <?php 
                        ...
                        {
                    ?>
                    <button type="button" id="payNow" onclick="payNow(this,'<?php echo $getPersonDetail[0]['aadharNumber']; ?>')" class="btn mt-0 btn-style-red disabled-on-loading" style="border-radius:0!important;padding:15px 50px!important;"> Continue</button>
                    <?php }else{ ?>
                    <button type="button" id="payNow" onclick="payWithout(this,'<?= $getPersonDetail[0]['Application_no']; ?>')" class="btn mt-0 btn-style-red disabled-on-loading" style="border-radius:0!important;padding:15px 50px!important;"> Continue</button>
                    <?php } ?>
                </div>
            </div>
        </div>
...
<script>
    ...
    function payWithout(ele,idNumber)
    {
        ...
        {
            $.ajax({
    		    url:'<?php echo base_url(); ?>user/saveLapataWithoutPayment',
                                ...
                                success:function(res)
                                {
                                    ...
                                    if(parse.finalCount > 0)
                                    {
                                        $.ajax({
                                            ...
                                            success:function(res)
                                            {
                                                location.href='<?php echo base_url(); ?>user/paymentWithoutExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                                            }
                                        })
                                    }else{
                                        location.href='<?php echo base_url(); ?>user/paymentWithoutExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    ...
                    var options = {
                    	...
                    	"image": "<?php echo base_url(); ?>assets/images/khoji.webp",
                    	...
                    	"prefill": 
                        {
                    		"name": "<?php echo $_SESSION['aadharName']; ?>",
                    		"contact": '<?php echo $_SESSION['loginmob']; ?>'
                    	},
                    	"theme": 
                        {
    ...
    function payNow(ele,idNumber)
    {
        ...
        {
            $.ajax({
    		    url:'<?php echo base_url(); ?>user/saveLapataUploadPayment',
    		    data:{unknownId:$('#unknownId').val(),'isUpload': <?= $isUpload; ?>,idNumber:idNumber,packageId:split[0],amount: split[1],type:'lapata','_token':'<?php echo $this->security->get_csrf_hash(); ?>'},
    		    type:'post',
    		    success:function(res)
    		    {
                        ...
                        success:function(res)
                        {
                            ...
                            if(parse.finalCount > 0)
                            {
                                $.ajax({
                                    ...
                                    success:function(res)
                                    {
                                        location.href='<?php echo base_url(); ?>user/paymentExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                                    }
                                })
                            }else{
                                location.href='<?php echo base_url(); ?>user/paymentExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                ...
                success:function(res)
                {
                    ...
                    var options = {
                    	...
                    	"handler": function (response) {
                    		$.ajax({
                    		    url:'<?php echo base_url(); ?>user/saveLapataUploadPayment',
                    		    data:{unknownId:$('#unknownId').val(),idNumber:idNumber,packageId:split[0],amount: split[1],type:'lapata','_token':'<?php echo $this->security->get_csrf_hash(); ?>'},
                    		    type:'post',
                    		    success:function(res)
                    		    {
                    		        let paymentResponse = JSON.parse(res);
                    	...
                    	modal:{
                    	    ...
                    	    {
                    	        $.ajax({
                    	            url:'<?= base_url(); ?>user/savePaymentStatus',
