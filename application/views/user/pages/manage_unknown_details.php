...
<main id="main" class="main">
    <div class="pagetitle row align-items-center">
        ...
        <div class="col-sm-12 text-end col-md-6 mb-3">
            <a href="<?= base_url(); ?>user/manageGallery?type=missing" class="btn btn-primary next text-light d-none" style="padding:12px 35px!important;">Search Person By Face Photo</a>
            <a href="<?= base_url(); ?>user/manageDraft?type=unknown" class="btn btn-primary save py-3">Manage Draft</a>
        </div>
                ...
                <div class="image-container row">
                    <?php
                        foreach($data as $row)
                        {
                            ...
                            $status = 'Inactive';
                            $isPaymentDone = 0;
                            $getLastTransaction = $this->db->query('select * from uploadtransactions where userId='.$_SESSION['user_id'].' and isKyc=1 and idNumber="'.$row['Application_no'].'" order by Id desc')->result_array();
                            ...
                            <div class="card mb-3" >
                                <div class="card-body px-0 h-100 pb-0">
                                    ...
                                    <div class="row col-12 px-1 mx-0 pt-2 text-light" style="max-height:auto;background:green;">
                                        ...
                                        <p><b style="font-weight:500;">Want to upgrade your plan- <a href="<?php echo base_url(); ?>user/showUnknownPaymentModels?id=<?php echo $row['id']; ?>" class="text-white" style="text-decoration:underline;">Click here</a></b></p>
                                    </div>
                                    <?php }else{ ?>
                                    <div class="row col-12 px-1 mx-0 pt-2 text-light" style="max-height:auto;background:red;">
                                        ...
                                        <p><b style="font-weight:500;">Want to Renew Your Plan- <a href="<?php echo base_url(); ?>user/showUnknownPaymentModels?id=<?php echo $row['id']; ?>" class="text-white" style="text-decoration:underline;">Click here</a></b></p>
                                    </div>
                                    <?php } ?>
                                <?php }else{ ?>
                                    <?php 
...
function edit(ele,appNo)
{
    ...
    location.href="<?php echo base_url(); ?>user/editUnknownDetails?id="+appNo;
}
...
function showColorAccordingly(isExpired,ele,id,appNo)
{
    if(ele.checked)
    {
        if(!isExpired)
        {
            ...
            location.href="<?php echo base_url(); ?>user/showUnknownPaymentModels?id="+id;
        }else{
            Swal.fire({
...
function checkApp(ele)
{
	...
	if(value != '')
	{
	    location.href="<?= base_url(); ?>user/manage_unknown_details?appNo="+value;   
	}else{
	    location.href="<?= base_url(); ?>user/manage_unknown_details";
	}
}
