...
<main id="main" class="main">
     <div class="card p-3">
         <div class="row align-items-center">
            <div class="col-sm-12 col-lg-7 col-md-6">
                <h4 class="text-md-end text-center text-danger mb-0">Select Your Plan</h4>
            </div>
         </div>
    </div>
    <?php 
        $isUpload = 0;
        if($_SERVER['HTTP_REFERER'] != base_url().'user/upload_unknown_details?pn=3')
        {
    ?>
        <?php 
            if($_SERVER['HTTP_REFERER'] != base_url().'user/manage_unknown_details')
            {
                $getId = parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY);
                $explode = explode('&',$getId);
        ?>
        <input type="hidden" id="redirect" value="<?php echo base_url().'user/editUnknownDetails?'.$explode[0].'&pn=4'; ?>">
        <?php      
            }else{
        ?>
    ...
    <?php }else{ 
        $isUpload = 1;
    ?>
    <input type="hidden" id="redirect" value="<?php echo base_url().'user/upload_unknown_details?pn=4'; ?>">
    <?php } ?>
    <div class="container-fluid px-0">
        <div class="row justify-content-center">
            <?php 
                $getPackages = $this->db->query('select * from packages where type="unknown" order by Id asc')->result_array();
                foreach($getPackages as $k=>$v)
                {
                    ...
                    if(count($getPersonDetail) > 0)
                    {
                        ...
                        {
                            $getIfAlready = $this->db->query('select * from uploadtransactions where isKyc=1 and idNumber="'.$getPersonDetail[0]['Application_no'].'" and type="unknown"')->result_array();      
                        }else{
                            $getIfAlready = array();
                        }
                    }else{
                    ...
                    $getAllotedPackages = $this->db->query('select * from allotedPackages where userType="Kyc" and userId='.$_SESSION['user_id'].' and packageId='.$v['Id'].' and complaintType="Unknown"')->result_array();
            ...
            ?>
            <div class="col-sm-12 col-lg-5 col-xl-3 col-md-5 me-md-3 mt-0 card px-0">
                <?php 
                    if($k == 2)
                    {
                ?>
                        ...
                        <div class="col-auto px-0">
                            ...
                            <?php 
                                ...
                                if($getIfFreeCredit[0]['freeUnknownCreditUsed'] == NULL)
                                {
                                    ...
                                    if($getIfFreeCredit[0]['freeUnknownCreditUsed'] < 20)
                                    {
                                        $freeCredit = 20 - $getIfFreeCredit[0]['freeUnknownCreditUsed'];   
                                    }
                                }
                                if($freeCredit != 0)
                                {
                            ?>
                            <h6 class="text-center text-light mb-0"><b style="font-family:math;">Credits Left : - <?= $freeCredit; ?> / 20</b></h6>
                            <?php } ?>
                            <?php } ?>
                            <?php }else{ ?>
    		        ...
    		        Swal.fire({
                        ...
                        $.ajax({
                            ...
                            success:function(res)
                            {
                                ...
                                {
                                    location.href='<?php echo base_url(); ?>user/view_expiry_unknown?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                                }else{
                                    location.href='<?php echo base_url(); ?>user/view_expiry_unknown?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                                }
                            }
