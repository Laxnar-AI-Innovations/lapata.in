                ...
                <div class="card mb-3 py-3 w-100 p-2">
                    <div class="card-body">
                        <div class="pt-0 pb-2 <?php if($pn == 2){ echo "d-none"; } ?>">
                            <h5 class="card-title text-center pb-0 fs-4 pt-1">Search Vehicle Details</h5>
                        </div>
                        <form class="row g-3 p-2 needs-validation <?php if($pn != 1){ echo "d-none"; } ?>" style="border:3px solid black;" method="post" action="<?php echo base_url(); ?>user/searchVehicle/step1" novalidate id="step1Form">
                            <?php 
                                $csrf = array(
...
<script type="text/javascript">
    ...
    {
        window.open('<?php echo base_url(); ?>user/checkVehicle2','_blank');
    }
                ...
                success:function(res)
                {
                    ...
                    if(parse.status == 'success')
                    {
                        ...
                        if(parse.status == 'error')
                        {
                            ...
                            {
                                Swal.fire({
                                    title: '<strong class="text-danger" style="font-size:20px;">No Records found</strong>',
                                    icon: 'error',
                                    allowOutsideClick:false,
                                    showCloseButton: true,
                                    showCancelButton: false,
        ...
        if(count == arr.length)
        {
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    ...
                    if(parse.status == 'success')
                    {
                        location.href="<?php echo base_url(); ?>user/searchVehicle?pn=2";   
                    }else{
                        toastr.error(parse.message);
                        $('#otp').val('');
