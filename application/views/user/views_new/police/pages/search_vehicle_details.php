                ...
                <div class="row justify-content-center mt-5">
                    <div class="col-md-auto col-lg-auto col-sm-12 mt-2">
                        <?php 
                            if($this->session->userdata('user_login') == 'yes'){
                        ?>
                        <a href="<?php echo base_url(); ?>police/search_vehicle_details" class="btn btn-primary next w-100" style="width:max-content;"><i class="fa fa-search"></i> Search Another</a>
                        <?php }else{ ?>
                            <a href="<?php echo base_url(); ?>police/search_vehicle_details" class="btn btn-primary next w-100" style="width:max-content;"><i class="fa fa-search"></i> Search Another</a>
                        <?php } ?>
                    </div>
                    <div class="col-md-auto col-lg-auto col-sm-12 mt-2">
                        <a href="<?php echo base_url(); ?>police" class="btn btn-primary save" style="width:-webkit-fill-available;"><i class="fa fa-home"></i> Homepage</a>
                    </div>
                </div>
            </div>
                <?php }else{ 
  ...
  <script type="text/javascript">
    ...
    $('#btnInactiveSave').click(function()
    {
        if($('#mainPhotoSelected').val() != '')
        {
            ...
            if(count == 0)
            {
                ...
                $.ajax({
                    url:'<?php echo base_url(); ?>police/checkInactiveVehicleIdAfterLogin',
                    data:data,
                    processData:false,
                    cache:false,
                    contentType:false,
                    ...
                    success:function(res)
                    {
                        ...
                        {
                            location.href="<?php echo base_url(); ?>police/search_vehicle_details?pn=2";
                        }
                        else{
    ...
    $('#step2Check').click(function()
    {
        ...
        if(count == 0)
        {
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>police/checkVehicleIdAfterLogin',
                data:data,
                processData:false,
                cache:false,
                contentType:false,
                        ...
                        if(parse.isVisible == '1')
                        {
                            if(parse.isRegistered == 0 && parse.isMissing == 1)
                            {
                                 ...
                                 Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        location.href="<?php echo base_url(); ?>police/checkVehicle2";
                                    }else if(result.isDismissed)
                                    {
                                        location.reload();
                                    }
    ...
    {
        window.open('<?php echo base_url(); ?>police/search_vehicle_details?pn=2','_blank');
    }
