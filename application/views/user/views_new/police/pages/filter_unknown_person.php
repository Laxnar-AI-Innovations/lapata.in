<style>
    .close{
    position: relative!important;
    color: #fff!important;
    font-size: 20px!important;
    line-height: 20px!important;
    font-weight: 600!important;
    padding: 10px 10px!important;
    background-color:black!important;
}

body {
  background: #ebedf2;
  font-family: "Poppins", sans-serif;
}

.empty-state {
  width: 100%;
  background: #ffffff;
  border-radius: 4px;
}

.empty-state__icon {
  width: 200px;
  height: 200px;
  display: flex;
  align-items: center;
  border-radius: 200px;
  justify-content: center;
  background-color: #f7fafc;
  box-shadow: 0px 2px 1px #e1e3ec;
}

.empty-state__content img {
    width: 170px;
}

.empty-state__message {
  color: #38a169;
  font-size: 1.5rem;
  font-weight: 500;
  margin-top: 0.85rem;
}

.empty-state__help {
  color: #a2a5b9;
  font-size: 0.875rem;
}

.empty-state__content {
    padding: 48px;
    display: flex;
    align-items: center;
    flex-direction: column;
}

.credit {
  color: #A2A5B9;
  font-size: .75rem;
  text-align: center;
  
  a {
    color: #444;
  }
}
</style>
<main id="main" class="main">

    <div class="pagetitle">
      <h5>Welcome, <?= $_SESSION['thanaName']; ?></h5>
    </div><!-- End Page Title -->

    <section class="section dashboard">
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body pt-3">
                        <div class="row" style="justify-content: space-evenly;">
                            <div class="col-md-6 col-sm-12 text-center mt-2">
                                <h4 class="text-danger mb-0">Search Unknown Person</h4>
                            </div>
                            <div class="col-md-auto col-sm-12 text-center mt-2">
                                <button type="button" class="btn btn-primary next w-100" data-bs-toggle="modal" data-bs-target="#filterModal">
                                    <i class="fa fa-filter"></i> Filter
                                </button>
                            </div>
                        </div>
                        <div class="modal fade" id="filterModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Filter Results</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                                </div>
                                <form class="row g-3 mx-0 needs-validation" method="post" action="<?php echo base_url(); ?>police/filterUnknownPerson" novalidate>
                                  <div class="modal-body pb-0 pt-1">
                                        <?php 
                                            $csrf = array(
                                            'name' => $this->security->get_csrf_token_name(),
                                            'hash' => $this->security->get_csrf_hash()
                                            );
                                        ?>
                                        <input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" />
                                        <div class="col-12">
                                            <div class="row">
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select Start Date</label>
                                                    <input type="date" class="form-control" name="startdate" id="startdate" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['startdate'] != '') ? $_POST['startdate'] : ''); ?>">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select End Date</label>
                                                    <input type="date" class="form-control" name="enddate" id="enddate" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['enddate'] != '') ? $_POST['enddate'] : ''); ?>">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select Gender</label>
                                                    <select class="form-control" name="gender" id="gender">
                                                        <option value="">Select Gender</option>
                                                        <option value="Male" <?php echo (($_POST['gender'] == 'Male') ? 'selected' : ''); ?>>Male</option>
                                                        <option value="Female" <?php echo (($_POST['gender'] == 'Female') ? 'selected' : ''); ?>>Female</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Enter Person Name</label>
                                                    <input type="text" class="form-control" name="personName" id="personName" value="<?php echo (($_POST['personName'] != '') ? $_POST['personName'] : ''); ?>" placeholder="Enter Person Name">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-0">
                                                    <label class="form-label">Person Father Name</label>
                                                    <input type="text" class="form-control" name="fatherName" id="fatherName" value="<?php echo (($_POST['fatherName'] != '') ? $_POST['fatherName'] : ''); ?>" placeholder="Person Father Name">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-0">
                                                    <label class="form-label">Select Dob</label>
                                                    <input type="date" class="form-control" name="dob" id="dob" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['dob'] != '') ? $_POST['dob'] : ''); ?>">  
                                                </div>
                                            </div>
                                        </div>
                                  </div>
                                  <div class="modal-footer">
                                        <div class="col-lg-auto col-md-auto col-sm-12 mb-3">
                                            <button class="btn btn-primary w-100 disabled-on-loading save" id="btn4" type="submit"> Check</button>  
                                        </div>
                                        <div class="col-lg-auto col-md-auto col-sm-12 mb-3">
                                            <button type="button" class="btn w-100 btn-secondary reset" data-bs-dismiss="modal">Close</button>
                                        </div>
                                  </div>
                                </form>
                            </div>
                          </div>
                        </div>
                        <hr>
                        <div class="row">
                            <?php 
                                if(count($finaldata) > 0)
                                {
                                    foreach($finaldata as $k=>$v)
                                    {
                            ?>
                            <div class="col-md-4 col-sm-12 col-lg-3 mt-2 text-center px-3">
                                <div class="w-100 card" style="cursor:pointer;" onclick="redirectToMissing(<?= $v['Id']; ?>)">
                                    <img class="card-img-top" src="<?php echo base_url(); ?><?php echo ($v['facematchphoto'] != NULL ? (file_exists($v['facematchphoto']) ? $v['facematchphoto'] : 'assets/img/noimage.jpg') : 'assets/img/noimage.jpg'); ?>" style="width:100%;height:200px;" alt="Card image cap">
                                    <div class="card-body">
                                        <h5 class="card-title"><?php echo $v['personName']; ?></h5>
                                        <p class="card-text"><?php echo $v['gender']; ?></p>
                                        <p class="card-text"><?php echo $v['dateCreated']; ?></p>
                                    </div>
                                </div>
                            </div>
                            <div class="modal fade" id="Modal<?php echo $v['Id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                              <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Person Detail</h5>
                                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                    </button>
                                    </div>
                                    <div class="modal-body pb-0 pt-1">
                                        <div class="col-12">
                                            <div class="row">
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select Start Date</label>
                                                    <input type="date" class="form-control" name="startdate" id="startdate" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['startdate'] != '') ? $_POST['startdate'] : ''); ?>">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select End Date</label>
                                                    <input type="date" class="form-control" name="enddate" id="enddate" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['enddate'] != '') ? $_POST['enddate'] : ''); ?>">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Select Gender</label>
                                                    <select class="form-control" name="gender" id="gender">
                                                        <option value="">Select Gender</option>
                                                        <option value="Male" <?php echo (($_POST['gender'] == 'Male') ? 'selected' : ''); ?>>Male</option>
                                                        <option value="Female" <?php echo (($_POST['gender'] == 'Female') ? 'selected' : ''); ?>>Female</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-2">
                                                    <label class="form-label">Enter Person Name</label>
                                                    <input type="text" class="form-control" name="personName" id="personName" value="<?php echo (($_POST['personName'] != '') ? $_POST['personName'] : ''); ?>" placeholder="Enter Person Name">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-0">
                                                    <label class="form-label">Person Father Name</label>
                                                    <input type="text" class="form-control" name="fatherName" id="fatherName" value="<?php echo (($_POST['fatherName'] != '') ? $_POST['fatherName'] : ''); ?>" placeholder="Person Father Name">  
                                                </div>
                                                <div class="col-md-6 col-sm-12 col-lg-6 mb-0">
                                                    <label class="form-label">Select Dob</label>
                                                    <input type="date" class="form-control" name="dob" id="dob" max="<?php echo date('Y-m-d'); ?>" value="<?php echo (($_POST['dob'] != '') ? $_POST['dob'] : ''); ?>">  
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <div class="col-lg-auto col-md-auto col-sm-12 mb-3">
                                            <button class="btn btn-primary w-100 disabled-on-loading save" id="btn4" type="submit"> Check</button>  
                                        </div>
                                        <div class="col-lg-auto col-md-auto col-sm-12 mb-3">
                                            <button type="button" class="btn w-100 btn-secondary reset" data-bs-dismiss="modal">Close</button>
                                        </div>
                                    </div>
                                </div>
                              </div>
                            </div>
                            <?php } }else{ ?>
                                <div class="col-lg-12 col-sm-12 col-md-12 text-center">
                                    <div class="empty-state">
                                      <div class="empty-state__content">
                                        <div class="empty-state__icon">
                                          <img src="data:image/png;base64," alt="">
                                        </div>
                                        <div class="empty-state__message">No persons found.</div>
                                      </div>
                                    </div>
                                </div>
                            <?php } ?>
                        </div>
                    </div>
                </div>
            </div><!-- End Left side columns -->
        </div>
    </section>

  </main><!-- End #main -->
  <script>
      function redirectToMissing(Id)
      {
          window.open('<?php echo base_url(); ?>landing/missingpersondetail/'+Id,'_blank');
      }
  </script>