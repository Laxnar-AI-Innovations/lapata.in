...
<?php 
    if(current_url() != base_url().'user/lapata_new_login' && $_SERVER['PATH_INFO'] != '/user/editLapataDetails' && current_url() != base_url().'user/add_family_member' && $_SERVER['PATH_INFO'] != '/user/editFamilyDetails' && current_url() != base_url().'user/upload_unknown_details' && $_SERVER['PATH_INFO'] != '/user/editUnknownDetails' && current_url() != base_url().'user/upload_personal_details')
    {
?>
<div style="height: 40px;"></div>
<div class="bottom-bar d-md-none d-flex text-center">
    <?php 
        if(current_url() == base_url().'user/newSearchPerson')
        {
    ?>
...
<br>Face Search</button>
    ...
    <?php 
        ...
        {
    ?>
    <button class="<?= current_url() == base_url().'user/missing_details' ? 'activeBottom' : ''; ?>" onclick="location.href='<?= base_url().''.$_SESSION['login_type']; ?>/missing_details'"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-upload"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2 -2v-2" /><path d="M7 9l5 -5l5 5" /><path d="M12 4l0 12" /></svg>
<br>Upload</button>
    <?php }else{ ?>
  ...
  <script type="text/javascript">
    ...
    <?php
        if($this->session->flashdata('already'))
        {
            ...
            {
                Swal.fire({
                    iconHtml: '<img src="<?= $this->session->flashdata('alreadyPhoto') ?>" style="height:100%;border-radius:50%;width:114px;">',
                    title:'<strong style="font-size:1rem;color:black;">Hello, <?= $this->session->flashdata('alreadyName'); ?></strong>',
                    customClass: {
                        icon: 'border-radius-50 w-7-em',
                        htmlContainer:'px-0 pt-0',
                        title:'pt-3',
                        popup:'w-auto',
                        actions:'mt-0'
                    },
                    allowOutsideClick:false,
                    html: '<div class="row mt-2 mx-0 justify-content-center" style="width:15em;"><div class="col-lg-12 col-md-12"><h6 style="color:red;font-weight:bold;">You are already a registered user.</h6></div>',
            ...
            <?php
                $this->session->unset_userdata('already');
                $this->session->unset_userdata('alreadyPhoto');
                $this->session->unset_userdata('alreadyName');
        }
    ?>
