            ...
            <div class="card-body p-3">
              ...
              <?php 
                    if(count($persondetail) > 0)
                    {
                        if($checkDetail[0]['idType'] == 'gold')
                        {
                            ...
                            {
                                $missingperson = $this->db->query('select * from missingperson where goldId="'.$checkDetail[0]['idNumber'].'" and isFound = 0 order by Id Desc')->result_array();   
                            }else{
                                $missingperson = array();
                            }
                        }else if($checkDetail[0]['idType'] == 'vehicle')
                        {
                            if($checkDetail[0]['isMissing'] == 1)
                            {
                                $missingperson = $this->db->query('select * from missingperson where vehicleNumber="'.$checkDetail[0]['idNumber'].'" and isFound = 0 order by Id Desc')->result_array();   
                            }else{
                                $missingperson = array();
                            }
                        }
                        else{
                            $missingperson = $this->db->query('select * from missingperson where missingaadhar="'.$persondetail[0]['aadhar_id'].'" and isFound = 0 order by Id Desc')->result_array();   
                        }
              ...
              <?php }else{ ?>
                  <form class="row g-3 needs-validation justify-content-center" method="post" action="<?php echo base_url(); ?>user/step2/do_register" novalidate id="registerForm">
                    ...
                    <?php } ?>
                    <?php 
                        if($persondetail[0]['wellrel1'] != NULL || $persondetail[0]['wellrel2'] != NULL || $persondetail[0]['wellrel3'] != NULL)
                        {
                    ?>
                    <?php 
                        if($this->session->userdata('user_login') == 'yes'){
                        ...
                        <div class="col-md-12 text-center border border-dark p-3">
                            <h6 class="text-dark">
                                ...
                                <a href="tel:<?php echo $persondetail[0]['well1']; ?>" id="contactspan" style="width:max-content;" class="phone btn btn-primary btn-style-green mt-2">
                                    <i class="fa fa-phone"></i> CALL to <?php echo ($persondetail[0]['wellrel1'] != 0 ? $getRelation[0]['relationName'] : 'Well Wisher'); ?>
                                </a>
                                <?php }
                                    if($persondetail[0]['wellrel2'] != NULL){ 
                                ...
                                <a href="tel:<?php echo $persondetail[0]['well2']; ?>" id="contactspan" style="width:max-content;" class="phone btn btn-primary btn-style-green mt-2">
                                    <i class="fa fa-phone"></i> CALL to <?php echo ($persondetail[0]['wellrel2'] != 0 ? $getRelation[0]['relationName'] : 'Well Wisher'); ?>
                                </a>
                                <?php }
                                    if($persondetail[0]['wellrel3'] != NULL){ 
                                ...
                                <a href="tel:<?php echo $persondetail[0]['well3']; ?>" id="contactspan" style="width:max-content;" class="phone btn btn-primary btn-style-green mt-2">
                                    <i class="fa fa-phone"></i> CALL to <?php echo ($persondetail[0]['wellrel3'] != 0 ? $getRelation[0]['relationName'] : 'Well Wisher'); ?>
                                </a>
                                <?php } ?>
                            </h6>
                        </div>
        ...
        <div class="container px-0">
            <div class="row justify-content-center align-items-center">
                <div class="card mb-1 w-100">
                    <div class="card-body p-3">
                        <div class="pb-0">
                            <h5 class="card-title mb-0 text-center pb-0 pt-0 text-danger">Wanted Records (<?php echo substr_replace($wanted[0]['aadharNumber'],'xxxx xxxx ',0,8); ?>)</h5>
                        </div>
                    </div>
                </div>
                <?php 
                ...
                <div class="col-lg-4 col-md-6 px-0 px-md-2 col-12 col-search pb-2">
                    <div class="card my-3 shiva_height">
                                ...
                                <div class="accordion" id="accordionExample<?php echo $k; ?>">
                                    <div class="accordion-item">
                                        <h2 class="accordion-header text-center mb-0" id="headingOne<?php echo $k; ?>">
                                            <button class="accordion-button btn btn-style-grey" style="background:transparent!important;color:black!important;" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne<?php echo $k; ?>" aria-expanded="true" aria-controls="collapseOne<?php echo $k; ?>">
