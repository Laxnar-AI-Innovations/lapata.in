...
<div class="container">
  ...
  <section class="section register d-flex flex-column justify-content-center my-5">
    <div class="container px-0">
      <div class="row justify-content-center">
        <div class="col-lg-4 col-md-6 d-flex flex-column align-items-center justify-content-center">

          <div class="card mb-3 p-0 w-100">
            <div class="card-body p-1">
                <div class="pb-1">
                <h5 class="mb-0 card-title text-center pb-0 fs-4 pt-2" style="font-size:1.2rem!important"><span style="font-size:1.2rem!important;font-weight:500;color:#63b881">Login</span> to Your Account</h5>
              </div>
                
                <div class="form-group mb-2 row g-3 text-center">
                    <div class="col-6 col-sm-12 col-md-12 col-lg-6">
                        <input class="radio-custom" type="radio" name="logintype" id="logingst" checked value="gst">
                        <label class="radio-custom-label" for="logingst">
                          GST No.
                        </label>
                    </div>
                    <div class="col-6 col-sm-12 col-md-12 col-lg-6 text-right">
                ...
                <div id="gstdiv" class="px-3">
                    <div class="form-floating mb-3 col-auto">
                      <input type="text" class="form-control" onkeydown="limit(this,'gst');" onkeyup="limit(this,'gst');" name="gstNumber" id="gstNumber" required placeholder="GST Number">
                      <label for="gstNumber">Enter - GST Number</label>
                      <div class="invalid-feedback">Please enter your GST number.</div>
                    </div>
                    <div class="form-group col-auto">
                        <button type="button" onclick="getRegistered('gst')" class="btn btn-primary btn-style-grey disabled w-100" id="btngstNext">Next <i class="fa fa-arrow-circle-right"></i></button>
                    </div>
                    <div class="col-auto d-none" id="mobilegstdiv">
                        <div class="form-floating mb-3">
                            <select name="gstmobileNumber" class="form-select" id="gstmobileNumber" aria-label="Select Mobile Number" onchange="toggleSendOtp(this.value)">
                                <option selected>Select Mobile Number</option>
                            </select>
                            <label for="aadharmobileNumber">Mobile Number</label>
                            <div class="invalid-feedback">Please enter your Mobile Number.</div>
                        </div>
                        <div class="col-lg-12 col-sm-12 col-md-12 mt-3 d-none" id="gstotpdiv">
                            <button type="button" class="w-100 btn btn-style-grey disabled-on-loading disabled" onclick="showOtp(this)"> Send OTP</button>
                        </div>
                    </div>
                </div>
                <div id="usernamediv" class="d-none px-3">
                    <div class="form-floating mb-3 col-auto">
                ...
                <div class="form-group col-auto mt-3 d-none" id="otpdiv">
                    <form action="<?php echo base_url(); ?>hotel/login/do_login" method="post" class="row gy-3 gx-0 needs-validation" novalidate id="loginform">
                        <?php 
                          $csrf = array(
...
<script type="text/javascript">
    ...
    function limit(val,type){
        if(type == 'gst')
        {
            ...
            if(val.value.length > max_chars) {
                $('#btngstNext').removeClass('btn-style-grey').addClass('btn-style-five');
                $('#btngstNext').removeClass('disabled');
                val.value = val.value.substr(0, (max_chars + 1));
            }else{
                $('#btngstNext').addClass('btn-style-grey').removeClass('btn-style-five');
                $('#btngstNext').addClass('disabled');
            }   
        }else{
            var min_chars = 0;
    ...
    function resendOtp(ele)
    {
        ...
        if($('#'+$('#type').val()+'mobileNumber').val() != '')
        {
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>hotel/sendLoginOtp',
                data:data,
                contentType: false,
                cache: false,
                processData:false,
    ...
    $('#loginform').on('submit',function(e)
    {
        ...
        if(arr[2].value == '')
        {
            toastr.error('Please Enter GST/ Username Value');
            return false;
        }
        else if(arr[3].value == '')
        {
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    ...
                    if(parse.status == 'success')
                    {
                        ...
                        if(type == 'aadhar')
                        {
                            $('#gstmobileNumber').attr('disabled','disabled');
    ...
    function getRegistered(type)
    {
        ...
        if(type == 'gst')
        {
            if(value.length == 15)
            {
                ...
                $.ajax({
                    url:'<?php echo base_url(); ?>hotel/getMobileDetails',
                    data:data,
                    contentType: false,
                    cache: false,
                    processData:false,
    ...
    $('input[name="logintype"]').on('change',function()
    {
        ...
        if(checked == 'gst')
        {
            ...
            $('#gstmobileNumber').attr('required',true);
        }else{
            $('#gstdiv').addClass('d-none');
            $('#username').attr('required',true);
            $('#usermobileNumber').attr('required',true);
            $('#gstNumber').attr('required',false);
            $('#gstmobileNumber').attr('required',false);
        }
    })
</script>