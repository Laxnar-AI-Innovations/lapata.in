...
<main id="main" class="main">
    ...
    <div class="pagetitle">
        <h1>Allot Search Package</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php echo base_url(); ?>admin">Home</a></li>
                <li class="breadcrumb-item active">Package</li>
                <li class="breadcrumb-item active">Allot Search Package</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
            ...
            <div class="col-xl-6">
                <div class="card">
                    <div class="card-body pt-3">
                        <form id="addDl" method="post" action="<?= base_url(); ?>admin/postAllotSearchPackage">
                            <?php 
                                $csrf = array(
                ...
                <div class="card">
                    ...
                    <div class="card-body pt-3 table-responsive">
                        <table class="table table-striped table-bordered datatable text-center">
                            ...
                            <tbody>
                                <?php 
                                    $getAllotedPackages = $this->db->query('select * from allotedsearchpackages')->result_array();
                                ...
                                <?php } }else{ ?>
                                <tr>
                                    <td colspan="4">No Packages Alloted...</td>
                                </tr>
                                <?php } ?>
                            </tbody>
                        </table>
  ...
  <script type="text/javascript">
    ...
    function deleteCheckedPackage(ele)
    {
        ...
        if(checked.length > 0)
        {
            if(confirm('Are you sure you want to delete this alloted packages?'))
            {
                ...
                $.ajax({
                    url:'<?php echo base_url(); ?>admin/deleteCheckedAllotedSearchPackages',
                    data:{'Ids[]':Ids,_token:'<?=$csrf['hash'];?>'},
                    type:'post',
                    success:function(res)
                    {
                        toastr.success('Alloted Search Packages Deleted Successfully');
                        setTimeout(function()
                        {
                          location.reload();
                        },2000);
    ...
    {
        if(confirm('Are you sure you want to delete this alloted search package?'))
        {
            $.ajax({
                url:'<?php echo base_url(); ?>admin/deleteAllotedSearchPackage',
                data:{Id:Id,_token:'<?=$csrf['hash'];?>'},
                type:'post',
                success:function(res)
                {
                    toastr.success('Alloted Search Package Deleted Successfully');
                    setTimeout(function()
                    {
                      location.reload();
                    },2000);
