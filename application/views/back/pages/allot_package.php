                        ...
                        <form id="addDl" method="post" action="<?= base_url(); ?>admin/postAllotPackage">
                            ...
                            <div class="form-floating form-group">
                                <select class="form-control" required id="complaintType" name="complaintType">
                                    ...
                                    <option value="lapata">Missing</option>
                                    <option value="unknown">Unknown</option>
                                    <option value="Both">Both</option>
                                </select>
                                <label for="complaintType">Choose Complaint Type?</label>
                            </div>
                            <div class="form-group pDiv d-none">
                ...
                <div class="card">
                    ...
                    <div class="card-body pt-3 table-responsive">
                        <table class="table table-striped table-bordered datatable text-center">
                            ...
                            <tbody>
                                <?php 
                                    $getAllotedPackages = $this->db->query('select * from allotedPackages')->result_array();
                                    if(count($getAllotedPackages) > 0)
                                    {
                                        $i=0;
                                        foreach($getAllotedPackages as $k=>$v)
                                        {
                                            $getPackageDetails = $this->db->query('select * from packages where Id='.$v['packageId'])->result_array();
                                            if($v['userType'] == 'Kyc')
                                            {
                                ...
                                <tr>
                                    ...
                                    <td><?= $getPackageDetails[0]['packageName']; ?></td>
  ...
  <script type="text/javascript">
    ...
    $('#complaintType').change(function()
    {
        if($('#complaintType').val() == '')
        {
            ...
            $.ajax({
                url:'<?= base_url(); ?>admin/getPackageAccordingly?type='+$('#complaintType').val(),
