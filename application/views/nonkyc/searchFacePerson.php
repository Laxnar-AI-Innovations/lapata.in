          ...
          <div class="card mb-3 py-3 px-0 w-100">
            ...
            <div class="card-body">
              ...
              <div class="pt-0 pb-2">
                ...
                <form class="row g-3 needs-validation <?php if($pn != 1){ echo "d-none"; } ?>" method="post" action="<?php echo base_url(); ?>user/searchFacePerson/step1" id="step1Form">
                    <?php 
                      $csrf = array(
                    ...
                    <div class="form-group mb-0">
                        <input type="radio" name="type" style="width:1.5rem;height:1.5rem;" id="type1" value="1" required>
                        <label for="type1" style="font-size:1.5rem!important;"><span class="text-danger">By-</span> Face Match</label>
                    </div>
                    <div class="form-group mt-0">
                        <input type="radio" name="type" style="width:1.5rem;height:1.5rem;" id="type2" value="2" required>
                        <label for="type2" style="font-size:1.5rem!important;"><span class="text-danger">By-</span> Fingerprint</label>
                    </div>
                ...
                <form class="row g-3 needs-validation <?php if($pn != 2){ echo "d-none"; } ?>" method="post" action="<?php echo base_url(); ?>user/searchFacePerson/step2" novalidate id="step2Form">
                    <?php 
                      $csrf = array(
                        'name' => $this->security->get_csrf_token_name(),
                        'hash' => $this->security->get_csrf_hash()
                ...
                <form class="row g-3 needs-validation justify-content-center <?php if($pn != 3){ echo "d-none"; } ?>" method="post" action="<?php echo base_url(); ?>user/searchFacePerson/step3" novalidate id="step3Form">
                    <?php 
                      $csrf = array(
                        'name' => $this->security->get_csrf_token_name(),
                        'hash' => $this->security->get_csrf_hash()
                    ...
                    <div class="row mt-3">
                        <div class="col-lg-12">
                            <h6 class="card-title text-dark text-center pb-0 fs-5 pt-1">Upload Search Person's Image</h6>
                        </div>
                    </div>
                    ...
                    <div class="row mt-3 justify-content-center" id="uploadBtnDiv">
                        ...
                        <div class="col-lg-10 col-md-6 text-center">
                            <button type="button" class="btn w-100 btn-primary btn-style-four" id="uploadPhotoBtn" onclick="showFileUpload()">Upload Photo <i class="fa fa-file-upload"></i></button>
                        </div>
                    </div>
                    <div class="row mt-4 justify-content-center">
                        <div class="col-6 px-1 d-none" id="retakebtndiv">
                            <button type="button" class="btn w-100 btn-primary btn-style-yellow" id="retakePhotoBtn" onclick="showFileUpload()">Retake <i class="fa fa-file-upload"></i></button>
                        </div>
                ...
                <form class="row g-3 p-3 align-items-center justify-content-center <?php if($pn != 4){ echo "d-none"; } ?>" method="post" action="<?php echo base_url(); ?>user/searchFacePerson/step4" id="step4Form">
                    <?php 
                      $csrf = array(
                        'name' => $this->security->get_csrf_token_name(),
                        'hash' => $this->security->get_csrf_hash()
...
<script type="text/javascript">
            ...
            success:function(res)
            {
                ...
                if(parse.status == 'success')
                {
                    if(parse.count > 0)
                    {
                        ...
                        if(parse.count > 1)
                        {
                            var html = '<div class="col-lg-12 col-sm-12 col-md-12 mb-3"><h5 class="text-center text-success">' + parse.count +' Persons Data Matched Found...</h5></div>';   
                        }else{
                            var html = '<div class="col-lg-12 col-sm-12 col-md-12 mb-3"><h5 class="text-center text-success">' + parse.count +' Person Data Matched Found...</h5></div>';
                        }
                        // html += '<div class="col-lg-12 col-sm-12 col-md-12 mb-3 border border-danger p-2" style="font-weight:bold;"><span class="text-danger">Note:- </span>Above Found Matched Data Will be Matched With The Person <span class="text-danger">Photo </span>After Clicking The <span class="text-danger">Search Button. </span></div>';
                        html += '<div class="col-lg-12 col-sm-12 col-md-12 text-left mb-4"><a style="white-space:break-spaces;width:100%;" target="_blank" href="<?php echo base_url(); ?>user/completeFaceMatchResults" class="btn btn-primary btn-style-green">Search result with face match filter <i class="fa fa-search"></i></a></div>';
                        html += '<div class="col-lg-12 col-sm-12 col-md-12 text-right"><a style="white-space:break-spaces;width:100%;background:black!important;" href="javascript:void(0)" onclick="closeSwal()" class="btn btn-dark btn-style-four">Add More Filter <i class="fa fa-plus-circle"></i></a></div>';
                        Swal.fire({
                          icon: 'success',
    ...
    $('#btn4Back').click(function()
    {
        ...
        location.href="<?php echo base_url(); ?>user/searchFacePerson?pn=1";
    })
    ...
    $('#step3Form').on('submit',function(e)
    {
        ...
        if(($('#mainPhotoSelected').val()) != '')
        {
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    location.href="<?php echo base_url(); ?>user/searchFacePerson?pn=4";   
                }
            });
        }else{
            Swal.fire({
        ...
        if(count == arr.length)
        {
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    ...
                    if(parse.status == 'success')
                    {
                        location.href="<?php echo base_url(); ?>user/searchFacePerson?pn=3";   
                    }else{
        ...
        if(count == arr.length)
        {
            if($('input[name="type"]:checked').val() != 2)
            {
                ...
                $.ajax({
                    ...
                    success:function(res)
                    {
                         location.href="<?php echo base_url(); ?>user/searchFacePerson?pn=2";
                    }
                });   
            }else{
                Swal.fire({
