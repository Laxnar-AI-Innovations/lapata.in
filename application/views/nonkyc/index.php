...
<html class="no-js">
    ...
    <?php 
        if(current_url() == base_url().'NonKyc/newUnknownFaceMatchResults')
        {
    ?>
    <body style="visibility:hidden;background: #fff;">
    <?php }else{ 
        if(current_url() == base_url().'NonKyc/newHighFaceMatchResults' || current_url() == base_url().'NonKyc/newFunnelFaceMatchResults')
        {
    ...
    <?php } } ?>
        ...
        <div id="ajaxloader" class="center d-none" style="flex-direction: column;">
            ...
            <h3 class="text-center">Please Wait <br><b class="dot-typing"></b></h3>
        </div>
        <?php 
            if(current_url() == base_url().'NonKyc/newUnknownFaceMatchResults')
            {
