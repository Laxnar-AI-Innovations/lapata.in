...
<main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Welcome, <span class="text-success"><?php echo $_SESSION['name']; ?></span></h1>
    </div><!-- End Page Title -->
            ...
            <div class="card-body w-100">
              ...
              <div class=" pb-2">
                ...
                <div class="tab-content">
                    <?php $getProfileDetail = $this->db->query('select * from hotel where hotel_id='.$_SESSION['hotel_id'])->result_array(); ?>
                    <div class="tab-pane fade profileTab active show" id="profileTab">
                       
                        <hr class="mb-4">
                        <form class="row g-3" method="post" action="<?php echo base_url(); ?>hotel/step2/do_register" id="registerForm">
                    <?php 
                      $csrf = array(
                    ...
                    <div class="row">
                        <div class="col mb-3">
                          <label for="aadharNumber" class="form-label">GST ID</label>
                          <input type="text" name="aadhar_id" readonly class="form-control" id="aadharNumber" required value="<?php echo $_SESSION['gstNumber']; ?>">
                        </div>
                        <div class="col mb-3">
                        ...
                        <div class="col mb-3">
                          ...
                          <input type="email" readonly name="email" class="form-control" id="email" value="<?php echo $getProfileDetail[0]['email']; ?>">
                          <div class="invalid-feedback">Please enter a valid Email</div>
                        </div>
        
                        <div class="col mb-3">
                            <label for="registermob" class="form-label">Registered Number</label>
                        ...
                        <div class="col mb-3">
                            <label for="registermob" class="form-label">Nearby Thana</label>
                            <?php 
                                if($getProfileDetail[0]['nearbyThana'] == '-')
                                {
                            ?>
                            <?php 
                                $getState = explode(',',$getProfileDetail[0]['address']);
                                $getDistrict = trim($getState[(count($getState)) - 3]);
                                $getThana = $this->db->query('select * from police_thana where state="'.trim($getState[(count($getState)) - 2]).'" and district="'.$getDistrict.'"')->result_array();
                                if(count($getThana) > 0)
                                {
                            ?>
                            <select class="form-control select2" name="nearbyThana" id="nearbyThana">
                                <option value="">Select Nearby Police Thana</option>
                                <?php 
                                    foreach($getThana as $k=>$v)
                                    {
                                ?>
                                <option value="<?= $v['thana']; ?>"><?= $v['thana'].','.$v['district']; ?></option>
                                <?php } ?>
                            </select>
                            <?php } else { ?>
                            <input type="text" readonly value="<?php echo $getProfileDetail[0]['nearbyThana']; ?>" class="form-control" name="nearbyThana">
                            <?php } ?>
                            <?php }else{ 
                                $getState = explode(',',$getProfileDetail[0]['address']);
                                $getDistrict = trim($getState[(count($getState)) - 3]);
                            ...
                            <select class="form-control select2" name="nearbyThana" id="nearbyThana">
                                ...
                                <?php 
                                    ...
                                    {
                                ?>
                                <option value="<?= $v['thana']; ?>" <?php echo (($getProfileDetail[0]['nearbyThana'] == $v['thana']) ? 'selected' : ''); ?>><?= $v['thana'].','.$v['district']; ?></option>
                                <?php } ?>
                            </select>
                            <!--<input type="text" readonly value="<?php echo $getProfileDetail[0]['nearbyThana']; ?>" class="form-control" name="nearbyThana">-->
                            <?php }else{ ?>
                    ...
                    <div class="row justify-content-center">
                        ...
                        <div class="col-sm-12 col-md-auto col-lg-auto mt-3">
                          <button class="btn btn-primary w-100 next" type="button" onclick="location.href='<?php echo base_url(); ?>hotel'">Go To Dashboard</button>
                        </div>
                        <!--<div class="col-sm-12 col-md-auto col-lg-auto mt-3">-->
...
<script type="text/javascript">
    ...
    function verifyDeleteOtp(ele,index)
    {
        ...
        if(otp != '')
        {
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>hotel/deleteAlternateMobile',
                data:data,
                contentType: false,
                cache: false,
                processData:false,
    ...
    function resendDeleteOtp(ele,index)
    {
        ...
        $.ajax({
            url:'<?php echo base_url(); ?>hotel/sendDeleteProfileOtp',
            data:otpdata,
            contentType: false,
            cache: false,
            processData:false,
    ...
    function verifyOtp(ele,index)
    {
        if($('#otp'+index).val() != '')
        {
            ...
            $.ajax({
              url:'<?php echo base_url(); ?>hotel/verifyProfileMobileOtp',
              data:data,
              contentType: false,
              cache: false,
              processData:false,
    ...
    function sendOtp(index)
    {
        if($('#othermob'+index).val() != '')
        {
            if($('#othermob'+index).val().length == 10)
            {
                ...
                data.append('_token','<?php echo $this->security->get_csrf_hash(); ?>');
                data.append('type','gst');
                data.append('typeval','<?php echo $_SESSION['aadhar_id']; ?>');
    
                $.ajax({
                  url:'<?php echo base_url(); ?>hotel/sendProfileMobileOtp',
                  data:data,
                  contentType: false,
                  cache: false,
                  processData:false,
