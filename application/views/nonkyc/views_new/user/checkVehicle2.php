        ...
        <div class="px-0 col-lg-10 col-xl-8 col-md-12 col-sm-12 d-flex flex-column align-items-center justify-content-center">
          ...
          <div class="card mb-3 w-100">
            ...
            <div class="card-body p-3">
                <div class="pb-2">
                    <h5 class="card-title text-center pb-0 fs-4 pt-2">Vehicle Details</h5>
                </div>
                <?php 
                    if($checkDetail[0]['searchType'] == 'vehicleNumber')
                    {
                        $missingvehicle = $this->db->query('select * from missingvehicle where rcNumber="'.$checkDetail[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound = 0')->result_array();
                    }
                    if($checkDetail[0]['searchType'] == 'chassisNumber')
                    {
                        $missingvehicle = $this->db->query('select * from missingvehicle where chasisNumber="'.$checkDetail[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound = 0')->result_array();
                    }
                    if($checkDetail[0]['searchType'] == 'engineNumber')
                    {
                        $missingvehicle = $this->db->query('select * from missingvehicle where engineNumber="'.$checkDetail[0]['idNumber'].'" and paymentStatus="Success" and isdeleted != 1 and isFound = 0')->result_array();
                    }
                ...
                <div class="row justify-content-center">
                    <div class="col-md-auto col-lg-auto col-sm-12 mt-3">
                        <?php 
                            if($this->session->userdata('user_login') == 'yes'){
                        ?>
                        <a href="<?php echo base_url(); ?>user/searchVehicle" class="btn btn-primary next w-100" style="width:max-content;"><i class="fa fa-search"></i> Search Another</a>
                        <?php }else{ ?>
                            <a href="<?php echo base_url(); ?>user/searchVehicle" class="btn btn-primary next w-100" style="width:max-content;"><i class="fa fa-search"></i> Search Another</a>
                        <?php } ?>
                    </div>
...
<script type="text/javascript">
    function showLogin()
    {
        Swal.fire({
          ...
          html:
            '<div class="form-group mb-2 row justify-content-center mx-0"><div class="col-lg-auto col-sm-12 col-md-auto mb-3"><a href="<?php echo base_url(); ?>user/login" class="w-100 save btn">Login</a></div><div class="col-lg-auto col-sm-12 col-md-auto"><a href="<?php echo base_url(); ?>user/checkvehicleregister" class="w-100 next btn">Signup</a></div></div></div>',
