...
<main id="main" class="main">
    ...
    <?php 
        $isUpload = 0;
        if($_SERVER['HTTP_REFERER'] != base_url().'NonKyc/lapata_login?pn=3')
        {
    ?>
        <?php 
            if($_SERVER['HTTP_REFERER'] != base_url().'NonKyc/manage_both_missing')
            {
                $getId = parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY);
                $explode = explode('&',$getId);
        ?>
        <input type="hidden" id="redirect" value="<?php echo base_url().'NonKyc/editLapataDetails?'.$explode[0].'&pn=4'; ?>">
        <?php      
            }else{
        ?>
    ...
    <?php }else{ 
        $isUpload = 1;
    ?>
    <input type="hidden" id="redirect" value="<?php echo base_url().'NonKyc/lapata_login?pn=4'; ?>">
    <?php } ?>
    <?php 
        ...
        if($getPersonDetail[0]['isFromUpload'] == 1)
        {
            $isUpload = 1;
        }
    ?>
    <div class="container-fluid px-0">
        <div class="row justify-content-center">
            <?php 
                $getPackages = $this->db->query('select * from packages where type="lapata" order by Id asc')->result_array();
                foreach($getPackages as $k=>$v)
                {
                    if(count($getPersonDetail) > 0)
                    {
                        ...
                        {
                            $getIfAlready = $this->db->query('select * from uploadtransactions where isKyc=0 and userId='.$_SESSION['user_id'].' and idNumber="'.$getPersonDetail[0]['aadharNumber'].'" and packageId=1')->result_array();      
                        }else{
                            $getIfAlready = array();
                        }
                    }else{
                    ...
                    $getAllotedPackages = $this->db->query('select * from allotedPackages where userType="NonKyc" and userId='.$_SESSION['user_id'].' and packageId='.$v['Id'].' and complaintType="Missing"')->result_array();
            ...
            <div class="col-sm-12 col-lg-3 col-md-5 me-md-3 mt-0 card px-0">
                    ...
                    <div class="row" style="justify-content:space-between;align-items:<?= ($v['amount'] == 0) ? 'baseline' : 'end'; ?>;">
                        ...
                        <div class="col-auto px-0">
                            ...
                            <?php 
                                ...
                                $freeCredit = 0;
                                if($getIfFreeCredit[0]['freeCreditUsed'] == NULL)
                                {
                                    $freeCredit = 1;
                                }
                                if($freeCredit != 0)
                                {
                            ?>
                            <h6 class="text-center text-light mb-0"><b style="font-family:math;">Credits Left : - <?= $freeCredit; ?> / 1</b></h6>
                            <?php } ?>
                            <?php } ?>
                            <?php }else{ ?>
                ...
                <div class="card-body descDiv p-0 align-items-center my-3" id="card-body<?= $v['Id']; ?>">
                    <ul type="disc" class="mb-0">
                        <?php 
                            if($k == 2)
                            {
                        ?>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/active.png" alt=""><span><b class="text-gradient gradient-theme d-inline">Your Missing Person data stays active for 3 Months</b><b> after successful purchase.</b></span></li>
                        ...
                        <?php }else{ ?>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/active.png" alt=""><span><b class="text-gradient gradient-theme d-inline">Your Missing Person data stays active for 1 Year</b><b> after successful purchase.</b></span></li>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/1.png" alt=""><span><b class="text-gradient gradient-theme">Anyone can search</b><b> your missing person using face or ID details uploaded by you.</b></span></li>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/2.png" alt=""><span><b class="text-gradient gradient-theme">Instant alert with alarm</b><b> if someone searches for your missing person.</b></span></li>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/3.png" alt=""><span><b class="text-gradient gradient-theme">Share link</b><b> of your missing person's details with ON/OFF control.</b></span></li>
                        <li style="color:black;font-size:0.9rem;"><img src="<?php echo base_url();?>assets_landing_page/price_icon/4.png" alt=""><span><b>Get alerts if NGOs, police, or volunteers upload your case for </b><b class="text-gradient gradient-theme">public identification.</b></span></li>
            ...
            <div class="col-sm-12 col-md-12 col-lg-12 text-center card">
                <div class="card-body p-0 py-3">
                    <?php 
                        ...
                        {
                    ?>
                    <button type="button" onclick="payNow(this,'<?php echo $getPersonDetail[0]['aadharNumber']; ?>')" class="btn mt-0 btn-style-red disabled-on-loading" style="border-radius:0!important;padding:15px 50px!important;"> Next</button>
                    <?php }else{ ?>
                    <button type="button" onclick="payWithout(this,'<?= $getPersonDetail[0]['Application_no']; ?>')" class="btn mt-0 btn-style-red disabled-on-loading" style="border-radius:0!important;padding:15px 50px!important;"> Next</button>
                    <?php } ?>
                </div>
            </div>
        </div>
...
<script>
    ...
    function payWithout(ele,idNumber)
    {
        ...
        {
            $.ajax({
    		    url:'<?php echo base_url(); ?>NonKyc/saveLapataWithoutPayment',
    		    data:{unknownId:$('#unknownId').val(),'isUpload': <?= $isUpload; ?>,idNumber:idNumber,packageId:split[0],amount: split[1],type:'lapata','_token':'<?php echo $this->security->get_csrf_hash(); ?>'},
    		    type:'post',
    		    success:function(res)
    		    {
    		        let paymentResponse = JSON.parse(res);
                    Swal.fire({
                        html:'<div class="row justify-content-center"><div class="col-12"><div id="lottie-popup-container" style="width:100%;height:200px;"></div></div></div><h6 style="text-align:center;color:black;font-weight:bold;" class="mb-2">Missing Person added successfully.</h6><h6 class="text-center"><b style="color:crimson;">Application No:- </b><b style="color:green;"><?= $getPersonDetail[0]['Application_no']; ?></b></h6><h6 class="text-center"><b style="color:crimson;">Person Name:- </b><b style="color:green;"><?= $getPersonDetail[0]['person_name']; ?></b></h6>',
                        willOpen : () => {
                    ...
                    }).then((result) => 
                    {
                        ...
                        $.ajax({
                            ...
                            success:function(res)
                            {
                                ...
                                {
                                    $.ajax({
                                        url:'<?= base_url(); ?>NonKyc/createOverallFaceHistory',
                                        success:function(res)
                                        {
                                            location.href='<?php echo base_url(); ?>NonKyc/paymentWithoutExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                                        }
                                    })
                                }else{
                                    location.href='<?php echo base_url(); ?>NonKyc/paymentWithoutExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
            ...
            rzpData.append('specialType','nonMissing');
            rzpData.append('unknownId', $('#unknownId').val());
            rzpData.append('isUpload', <?= $isUpload; ?>);
            rzpData.append('idNumber',idNumber);
            rzpData.append('packageId',split[0]);
            rzpData.append('type','lapata');
            ...
            $.ajax({
                ...
                success:function(rzpRes)
                {
                                    ...
                                    success:function(res)
                                    {
                                        ...
                                        if(parse.finalCount > 0)
                                        {
                                            $.ajax({
                                                ...
                                                {
                                                    $.ajax({
                                                        url:'<?= base_url(); ?>NonKyc/getLastInsertedMissingId?type=without',
                                                        success:function(insertResponse)
                                                        {
                                                            let parseInsert = JSON.parse(insertResponse);
                                                            location.href='<?php echo base_url(); ?>NonKyc/paymentWithoutExpiries?id='+parseInsert.Id+'&redirect='+$('#redirect').val();
                                                        }
                                                    })
                                                }
                                            })
                    ...
                    propay.on('payment.cancelled', function (response){
                        toastr.error(response.error.description);
                        $(ele).removeAttr('disabled');
                    });
                }
    		    ...
    		    success:function(res)
    		    {
                    ...
                    $.ajax({
                        ...
                        success:function(res)
                        {
                            ...
                            if(parse.finalCount > 0)
                            {
                                location.href='<?php echo base_url(); ?>NonKyc/paymentExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                            }else{
                                location.href='<?php echo base_url(); ?>NonKyc/paymentExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
                            }
                        }
                            ...
                            Swal.fire({
                                ...
                                $.ajax({
                                    ...
                                    success:function(res)
                                    {
                                        let parse = JSON.parse(res);
                                        $.ajax({
                                            url:'<?= base_url(); ?>NonKyc/getLastInsertedMissingId?type=with',
                                            success:function(insertResponse)
                                            {
                                                let parseInsert = JSON.parse(insertResponse);
                                                location.href='<?php echo base_url(); ?>NonKyc/paymentExpiries?id='+parseInsert.Id+'&redirect='+$('#redirect').val();
                                            }
                                        })
                                    }
                                });
