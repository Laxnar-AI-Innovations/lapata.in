                ...
                <div class="image-container row">
                    <?php
                        foreach($data as $row)
                        {
                            ...
                            $status = 'Inactive';
                            $isPaymentDone = 1;
                            if(strtotime(date('d-m-Y')) <= strtotime(date('d-m-Y',strtotime($v['expiryDate']))))
                            {
                                $isExpired = 1;
                            }
                          ?>
...
function checkApp(ele)
{
	...
	if(value != '')
	{
	    location.href="<?= base_url(); ?>NonKyc/manageDraft?type=funnelmissing&appNo="+value;   
	}else{
	    location.href="<?= base_url(); ?>NonKyc/manageDraft?type=funnelmissing";
	}
}
</script>
<?php }else{ ?>
...
function checkApp(ele)
{
	...
	if(value != '')
	{
	    location.href="<?= base_url(); ?>NonKyc/manageDraft?type=funnelfamily&appNo="+value;   
	}else{
	    location.href="<?= base_url(); ?>NonKyc/manageDraft?type=funnelfamily";
	}
}
</script>
<?php } ?>