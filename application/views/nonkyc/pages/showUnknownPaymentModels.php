...
<main id="main" class="main">
    ...
    <?php 
        $isUpload = 0;
        if($_SERVER['HTTP_REFERER'] != base_url().'NonKyc/upload_unknown_details?pn=3')
        {
    ?>
        <?php 
            if($_SERVER['HTTP_REFERER'] != base_url().'NonKyc/manage_unknown_details')
            {
                $getId = parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY);
                $explode = explode('&',$getId);
        ?>
        <input type="hidden" id="redirect" value="<?php echo base_url().'NonKyc/editUnknownDetails?'.$explode[0].'&pn=4'; ?>">
        <?php      
            }else{
        ?>
    ...
    <?php }else{ 
        $isUpload = 1;
    ?>
    <input type="hidden" id="redirect" value="<?php echo base_url().'NonKyc/upload_unknown_details?pn=4'; ?>">
    <?php } ?>
