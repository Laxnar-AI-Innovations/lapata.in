                ...
                <div class="row justify-content-center">
                    <div class="col-lg-10 col-md-12 d-flex flex-column align-items-center justify-content-center <?php if($pn != 1){ echo "d-none"; } ?>">
                        ...
                        <?php 
                            ...
                            {
                                $getLastTransaction = $this->db->query('select * from uploadtransactions where isKyc=0 and userId='.$_SESSION['user_id'].' and idNumber="'.$data[0]['aadharNumber'].'" order by Id desc')->result_array();
                                ...
                                $getLastTransaction = $this->db->query('select * from uploadWithoutTransactions where userId='.$_SESSION['user_id'].' and isKyc=0 and idNumber="'.$data[0]['Application_no'].'" order by Id desc')->result_array();
                                if(count($getLastTransaction) > 0)
                                {
                                    if(strtotime(date('d-m-Y')) <= strtotime(date('d-m-Y',strtotime($getLastTransaction[0]['expiryDate']))))
                                    {
                        ...
                        <form id="loginForm" action="<?php echo base_url(); ?>NonKyc/lapatawith" method="post" class="row w-100 g-3 needs-validation1"  enctype="multipart/form-data">
                            <?php 
                                $csrf = array(
                            ...
                            <?php 
                                ...
                                $getDetail = $this->db->query('select * from missingFaceMatchPayment where paymentStatus = 1 and unknownId='.$data[0]['id'].' order by Id desc')->result_array();
                            ...
                            <div class="card">
                                <div class="card-body mt-0">
                                    <div class="row align-items-center ">
                                        ...
                                        <div class="form-floating my-3 col-12 col-md-5">
...
<script>
    ...
    function payNow(ele)
    {
        ...
        if($('#terms').is(':checked'))
        {
            ...
            uniqueData.append('uniqueAadhar','<?= $data[0]['aadharNumber']; ?>');   
            uniqueData.append('Id','<?= $data[0]['id']; ?>');
                ...
                success:function(res)
                {
                    ...
                    if(parse.status == 'notapplicable')
                    {
                        Swal.fire({
                            ...
                            location.href='<?php echo base_url(); ?>NonKyc/showPaymentModels?type=missing&id=<?= $data[0]['id']; ?>';
                        });
                    }else{
                        location.href='<?php echo base_url(); ?>NonKyc/showPaymentModels?type=missing&id=<?= $data[0]['id']; ?>';
                    }
    ...
    $('#btn3Next').click(function(){
        ...
        $.ajax({
            ...
            success:function(res)
            {
                ...
                setTimeout(function()
                {
                    location.href="<?php echo base_url(); ?>NonKyc/editLapataDetails?id=<?php echo $data[0]['Application_no'] ?>&pn=4";
                },2000);      
            }
        })
    })
    ...
    {
        $(ele).attr('disabled',true);
        location.href="<?php echo base_url(); ?>NonKyc/manage_both_missing";
    }
    ...
    $('#btn3Back').click(function(){
        $(this).attr('disabled','disabled');
        location.href="<?php echo base_url(); ?>NonKyc/editLapataDetails?id=<?php echo $data[0]['Application_no'] ?>&pn=2";
    })
    
    $('#btn2Next').click(function(){
        $(this).attr('disabled','disabled');
        location.href="<?php echo base_url(); ?>NonKyc/editLapataDetails?id=<?php echo $data[0]['Application_no'] ?>&pn=3";
    })
    
    $('#btn2Back').click(function(){
        $(this).attr('disabled','disabled');
        location.href="<?php echo base_url(); ?>NonKyc/editLapataDetails?id=<?php echo $data[0]['Application_no'] ?>&pn=1";
    })
