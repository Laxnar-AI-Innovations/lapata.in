...
<main id="main" class="main">
    ...
    <?php 
        $isUpload = 0;
        if($_SERVER['HTTP_REFERER'] != base_url().'upload-missing-funnel?pn=3')
        {
    ?>
        <?php 
        ...
        ?>
        <input type="hidden" id="redirect" value="<?php echo base_url().'NonKyc/editFunnelMissingDetails?'.$explode[0].'&pn=4'; ?>">
        <?php      
            }else{
        ?>
    ...
    <?php }else{ 
        $isUpload = 1;
    ?>
    <input type="hidden" id="redirect" value="<?php echo base_url().'upload-missing-funnel?pn=4'; ?>">
    <?php } ?>
    <div class="container-fluid px-0">
        <div class="row justify-content-center">
            <?php 
                $getPackages = $this->db->query('select * from packages where type="lapata" and Id > 1 order by Id asc')->result_array();
        ...
        <input type="hidden" id="funnelId" value="<?= $_GET['id']; ?>">
        <div class="row justify-content-center">
            <div class="col-sm-12 col-md-12 col-lg-12 text-center card">
                <div class="card-body p-0 py-3">
                    <button type="button" onclick="payNow(this,'<?php echo $_GET['id']; ?>')" class="btn mt-0 btn-style-red disabled-on-loading" style="border-radius:0!important;padding:15px 50px!important;"> Next</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function payNow(ele,idNumber)
    {
        ...
        {
            $.ajax({
    		    url:'<?php echo base_url(); ?>NonKyc/saveFunnelMissingUploadPayment',
    		    data:{idNumber:idNumber,packageId:split[0],amount: split[1],'_token':'<?php echo $this->security->get_csrf_hash(); ?>'},
    		    type:'post',
    		    success:function(res)
    		    {
    		        location.href='<?php echo base_url(); ?>NonKyc/paymentFunnelMissingExpiries?id='+paymentResponse.Id+'&redirect='+$('#redirect').val();
    		    }
    		})   
        }else{
            ...
            $.ajax({
                ...
                success:function(rzpRes)
                {
                    ...
                    var options = {
                    	...
                    	"amount": (split[1] * 100),
                    	"name": "Khoji",
                    	"description": "Funnel Missing Payment",
