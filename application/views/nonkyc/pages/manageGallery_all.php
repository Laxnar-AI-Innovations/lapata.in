        ...
        <div class="row">
            <div class="col-md-6 col-12 text-center text-md-start">
                <?php 
                    ...
                    {
                ?>
                <h1>Search Unknown Person data by face photo filter</h1>  
                <?php }else{ ?>
                <h1>Search Missing Person data by face photo filter</h1>  
                <?php } ?>
            </div>
            <div class="col-md-6 col-12 text-center text-md-end d-none">
                                ...
                                <div class="row align-items-center px-0 mx-0 justify-content-center">
                                    <div class="col-10">
                                        <?php 
                                            ...
                                            {
                                        ?>
                                        <p class="mb-0 mt-1" style="color:red;"><b>MISSING</b></p>
                                        <?php }else{ ?>
                                        <p class="mb-0 mt-1" style="color:red;"><b>APPEAL FOR IDENTIFICATION</b></p>
                                        <?php } ?>
                                    </div>
                                    <div class="col-10">
                                    ...
                                    <div class="col-12 px-2">
                                        <p class="mb-1"><b style="color:black;font-size:.8rem!important;font-weight:500;">Rescue Date :- </b><b style="color:red;font-size:.8rem!important;"><?php echo date('d-m-Y',strtotime($row['rescueDate'])); ?></b></p>
                                    </div>
                                    <?php } ?>
                                    <?php 
                                        if($row['person_name'] != '')
                                    ...
                                    <div class="col-12 text-start imgDiv_<?= $row['id']; ?>">
                                        ...
                                        <div class="row">
                                            <div class="col-12 text-center">
                                                <button type="button" id="search_<?= $row['id']; ?>" onclick="search(this,<?= $row['id']; ?>,'<?= $row['awsEventName']; ?>')" class="btn btn-success save disabled-on-loading search" style="padding:0.6rem!important;text-transform:unset;"> <i class="fa fa-search"></i> with 1st Photo</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
?>
<?php 
    $checkIfAlready = $this->db->query('select * from guidePopup where userId='.$_SESSION['user_id'].' and isKyc=0 and url="'.current_url().'"')->result_array();
    ...
    if($('#selectedPhoto_'+id).val() == '')
    {
        ...
        $.ajax({
            ...
            success:function(res)
            {
                ...
                {
                    location.href="<?php echo base_url(); ?>NonKyc/completeFaceMatchResults";      
                }else{
                    Swal.fire({
                        ...
                        <?php 
                            ...
                            {
                        ?>
                        html: "<h6 style='font-weight:bold;color:black;font-size:0.743rem;' class='mt-1'><b>Face Not Found in <br><b style='color:red;text-transform:uppercase;'>Appeal For Identification</b> data</b></h6><div class='form-group mb-2'><button type='button' class='btn btn-success save' onclick='swal.close();'>Please Try Again</button></div><div class='form-group mb-0 text-end'><button type='button' class='btn btn-primary btn-style-primary-no-border'>View Guidlines</button></div>",
                        <?php }else{ ?>
                        html: "<h6 style='font-weight:bold;color:black;font-size:0.743rem;' class='mt-1'><b>Face Not Found in <br><b style='color:red;text-transform:uppercase;'>Missing Person</b> data</b></h6><div class='form-group mb-2'><button type='button' class='btn btn-success save' onclick='swal.close();'>Please Try Again</button></div><div class='form-group mb-0 text-end'><button type='button' class='btn btn-primary btn-style-primary-no-border'>View Guidlines</button></div>",
                        <?php } ?>
                        showCloseButton:true,
...
function checkApp(ele)
{
	...
	if(value != '')
	{
	    location.href="<?= base_url(); ?>NonKyc/manageGallery?appNo="+value+"&type=<?= $_GET['type']; ?>";   
	}else{
	    location.href="<?= base_url(); ?>NonKyc/manageGallery?type=<?= $_GET['type']; ?>";
	}
}
