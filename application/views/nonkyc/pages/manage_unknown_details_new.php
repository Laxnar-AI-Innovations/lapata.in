...
<main id="main" class="main">
    <div class="pagetitle row align-items-center">
        ...
        <div class="col-sm-12 text-end col-md-6 mb-3">
            <a href="<?= base_url(); ?>NonKyc/manageDraftNew?type=unknown" class="btn btn-primary save">Manage Draft</a>
        </div>
                ...
                <div class="image-container row">
                    <?php
                        foreach($data as $row)
                        {
                            ...
                            $status = 'Inactive';
                            $isPaymentDone = 0;
                            $getLastTransaction = $this->db->query('select * from uploadtransactions where userId='.$_SESSION['user_id'].' and isKyc=0 and idNumber="'.$row['Application_no'].'" order by Id desc')->result_array();
                        ...
                        <div class="col-lg-4 col-md-6 px-0 px-md-2 col-12 col-search pb-2" data-appNo="<?= $row['Application_no']; ?>" data-imageId="<?php echo $row['imageId']??"-" ?>" data-cat="<?php echo $row['DL_no']??"-" ?>" data-goldtype="<?= implode(',',$goldType); ?>" data-goldhuid="<?= implode(',',$goldHUID); ?>" data-gender="<?php echo $row['Gender']??"" ?>" data-vehicle="<?php echo $row['vehicle_no']??"" ?>" data-aadhar="<?php echo $row['aadharNumber']??"" ?>" data-pan="<?php echo $row['PAN']??"" ?>" data-voter="<?php echo $row['voter_id']??"" ?>" data-dl="<?php echo $row['DL_no']??"" ?>" data-age="<?php echo $row['Age']??"" ?>" data-state="<?php echo $row['state']??"" ?>" data-district="<?php echo $row['district']??"" ?>" data-city="<?php echo $row['city']??"" ?>" data-personname="<?php echo $row['person_name']??"" ?>"  data-search="<?php echo $row['DL_no']; ?><?php echo $row['aadharNumber']; ?><?php echo $row['PAN']; ?><?php echo $row['person_name']; ?><?php echo $row['Message']; ?>" >
                            <div class="card mb-3" >
                                <div class="card-body px-0 h-100 pb-0">
                                    ...
                                    <div class="row col-12 px-1 mx-0 pt-2 text-light" style="max-height:auto;background:green;">
                                        ...
                                        <p><b style="font-weight:500;">Want to upgrade your plan- <a href="<?php echo base_url(); ?>NonKyc/showUnknownPaymentModelsNew?id=<?php echo $row['id']; ?>" class="text-white" style="text-decoration:underline;">Click here</a></b></p>
                                    </div>
                                    <?php }else{ ?>
                                    <div class="row col-12 px-1 mx-0 pt-2 text-light" style="max-height:auto;background:red;">
                                        ...
                                        <p><b style="font-weight:500;">Want to Renew Your Plan- <a href="<?php echo base_url(); ?>NonKyc/showUnknownPaymentModelsNew?id=<?php echo $row['id']; ?>" class="text-white" style="text-decoration:underline;">Click here</a></b></p>
                                    </div>
                                    <?php } ?>
                                <?php }else{ ?>
                                    <?php 
...
function edit(ele,appNo)
{
    ...
    location.href="<?php echo base_url(); ?>NonKyc/editUnknownDetailsNew?id="+appNo;
}
...
function showColorAccordingly(isExpired,ele,id,appNo)
{
    if(ele.checked)
    {
        if(!isExpired)
        {
            ...
            location.href="<?php echo base_url(); ?>NonKyc/showUnknownPaymentModelsNew?id="+id;
        }else{
            Swal.fire({
...
document.querySelector('#search-box').onchange = () => {
	...
	if(value != '')
	{
	    location.href="<?= base_url(); ?>NonKyc/manage_unknown_details_new?appNo="+value;   
	}else{
	    location.href="<?= base_url(); ?>NonKyc/manage_unknown_details_new";
	}
};
