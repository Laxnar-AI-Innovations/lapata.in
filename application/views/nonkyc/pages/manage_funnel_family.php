...
<main id="main" class="main">
    <div class="pagetitle row align-items-center">
        ...
        <div class="col-sm-12 text-end col-md-6 mb-3">
            <a href="<?= base_url(); ?>NonKyc/manageDraft?type=funnelfamily" class="btn btn-primary save py-3">Manage Draft</a>
        </div>
    </div>
    <div>
            ...
            <div class="gallery px-0">
                <div class="image-container row">
                    <?php
                        foreach($data as $row)
                        {
                            $getGoldDetails = $this->db->query('select * from funnelfamilypersongold where funnelId='.$row['id'])->result_array();
                            $goldHUID = array();
                            $goldType = array();
                            ...
                            $status = 'Inactive';
                            $isPaymentDone = 1;
                            $getLastTransaction = $this->db->query('select * from funneltransactions where userId='.$_SESSION['user_id'].' and paymentType="family" order by Id desc')->result_array();
    ...
    if(ele.checked)
    {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:'<?php echo base_url(); ?>NonKyc/reactivateFunnelFamilyMember',
                    data:data,
                    contentType: false,
                    cache: false,
                    processData:false,
...
{
    window.open('<?php echo base_url(); ?>find-funnel-family-person?id='+id,'_blank');
}
...
function share(type,id)
{
    if (navigator.share) {
        navigator.share({
          title: 'Khoji',
          url: '<?php echo base_url(); ?>find-funnel-family-person?id='+id+'&type='+type,
...
{
    $(ele).attr('disabled','disabled');
    location.href="<?= base_url(); ?>NonKyc/manage_funnel_family";
}
